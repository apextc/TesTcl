...
# testcl::call --
#
# stub for the iRule GLOBAL::call
...
# testcl::decode_uri --
#
# stub for the iRule GLOBAL::decode_uri
proc ::testcl::decode_uri uri {
  return [URI::decode $uri]
}

# testcl::domain --
#
# stub for the iRule GLOBAL::domain
proc ::testcl::domain {fqdn count} {
  if {($fqdn ne "") && ($count ne "") && ([string is digit $count])} {
    log::log debug "GLOBAL domain invoked 'domain $fqdn $count'"
    set domainList [split $fqdn "."]
    return [join [lrange $domainList [expr {[llength $domainList] - $count}] end] "."]
  } else {
    log::log error "GLOBAL domain invoked with incorrect syntax 'domain $fqdn $count', usage domain <string> <count>"
  }
}
